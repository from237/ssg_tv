...
# ==========================================
# [메인] 실행 로직 (정합성 유지 + 5,6일 수집)
# ==========================================
def run():
    print(f"🚀 부분 업데이트 시작: {TARGET_DATES[0]} ~ {TARGET_DATES[-1]}")
    ...
    collected_data = [] # 데이터를 리스트에 모았다가 한 번에 저장 (오류 방지)
                ...
                for dl in dl_list:
                    ...
                    for card in cards:
                        try:
                            ...
                            # [중요] 사용자가 작성한 MD 분류 로직 적용
                            md_class = determine_md_class(brand, c1, c2)

                            # 딕셔너리 형태로 저장 (Pandas가 컬럼명 맞춰줌)
                            collected_data.append({
                                ...
                                "상품ID": gid, "이미지URL": i_url, "상세링크": link, 
                                "MD분류": md_class
                            })
                            cnt += 1
                        except:
    ...
    # ==========================================
    # [저장] 기존 data.csv 읽어서 5,6일만 교체 (Pandas 병합)
    # ==========================================
    new_df = pd.DataFrame(collected_data, columns=headers)
    ...
    if os.path.exists(FILENAME):
        try:
            ...
            # 2. 이번에 수집한 날짜들(5, 6일)이 기존 파일에 있다면 삭제 (중복 방지)
            target_dates_str = [d.strftime("%Y-%m-%d") for d in TARGET_DATES]
            old_df = old_df[~old_df['방송일자'].isin(target_dates_str)]
            
            # 3. 병합 (헤더 이름 기준 정렬 -> 컬럼 밀림 해결)
            final_df = pd.concat([old_df, new_df], ignore_index=True)
            print(" 🔄 기존 파일 업데이트 및 정렬 완료")
            
        except Exception as e:
            print(f" ⚠️ 기존 파일 읽기 실패, 새로 생성: {e}")
            final_df = new_df
    else:
        final_df = new_df
