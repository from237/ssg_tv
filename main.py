...
# ==========================================
# [메인] 실행 로직 (안전한 스마트 업데이트)
# ==========================================
def run():
    # -----------------------------------------------------------
    # [수정] 요청하신 1월 5일, 6일 데이터만 수집하도록 날짜 고정
    # -----------------------------------------------------------
    # 현재 연도를 기준으로 1월 5일, 6일 설정 (2026년 가정 시 2026, 1, 5)
    # 만약 실행 시점이 2026년이라면 그대로 둡니다.
    target_year = 2026 
    
    start_date = date(target_year, 1, 5) 
    end_date = date(target_year, 1, 6)
    
    print(f"🚀 [KST] 지정 날짜 업데이트 시작: {start_date} ~ {end_date}")
    ...
    if os.path.exists(FILENAME):
        try:
            main_df = pd.read_csv(FILENAME, encoding='utf-8-sig')
        except:
    ...
    for i in range(delta.days + 1):
        ...
        p_date = target_date.strftime("%Y/%m/%d") # URL용
        s_date = target_date.strftime("%Y-%m-%d") # 저장용
        
        print(f"   📅 {s_date} 확인 중...", end="")
        ...
        try:
            ...
            # [핵심 로직] 데이터가 1건이라도 수집되었을 때만 갱신
            if len(day_rows) > 0:
                # 1. 기존 데이터프레임에서 해당 날짜 제거 (중복 방지)
                main_df = main_df[main_df['방송일자'] != s_date]
                
                # 2. 새 데이터 추가
                new_day_df = pd.DataFrame(day_rows, columns=headers)
                main_df = pd.concat([main_df, new_day_df], ignore_index=True)
                print(f" ✅ {len(day_rows)}건 갱신 완료")
            else:
                print(" ⚠️ 수집 데이터 0건 -> 기존 데이터 유지")
                
        except Exception as e:
            print(f" ❌ 에러 발생 ({e}) -> 기존 데이터 유지")

    # 최종 정렬 및 저장
    main_df = main_df.sort_values(by=['방송일자', '방송시간'])
    main_df.to_csv(FILENAME, index=False, encoding='utf-8-sig')
    print(f"🎉 최종 저장 완료: {FILENAME} (총 {len(main_df)}건)")
